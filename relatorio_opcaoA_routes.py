"""
Cole o conteúdo deste arquivo dentro do seu app.py
(ou importe as funções) para habilitar o relatório
de PARCELAS PAGAS SEM tocar na coluna 'vencimento'.

Requisitos no arquivo principal (app.py):
- Variáveis/objetos existentes: app, db, Contrato, Parcela
- Imports:
...
def relatorio_parcelas_pagas():
    ...
    if request.method == 'POST':
        try:
            # inputs <input type="date"> -> 'YYYY-MM-DD'
...
def exportar_relatorio_parcelas_pagas():
    # recebe ?inicio=YYYY-MM-DD&fim=YYYY-MM-DD
    try:
